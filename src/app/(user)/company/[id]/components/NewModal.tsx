...
const Modal = ({ id }) => {
  ...
  const [topResumeFileId, setTopResumeFileId] = useState(null);
  const TopResumes = api.company.getCompanyJobsTopresume.useMutation({
    onSuccess: (data) => {
      if (data) {
        console.log(data);
setTopResumeFileId(data[0])
      }
...
{topResumeFileId && console.log(topResumeFileId)
}
      <AnimatePresence>
        {isModalOpen && (
          <motion.div
          ...
          >
            ...
            <div className=" bg-white max-w-lg mx-auto rounded shadow-lg z-50 overflow-y-auto">
              ...
              <div className=" min-w-[600px] flex  flex-col  p-8">
                  ...
                  {topResumeFileId && topResumeFileId.jobs.map((job) => (
